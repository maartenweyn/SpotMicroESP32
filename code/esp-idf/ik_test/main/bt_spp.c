...
static uint8_t raw_adv_data[] = {
        ...
        0x03, 0x03, 0xFF, 0x00,
        /* device name */
        0x0f, 0x09, 'E', 'S', 'P', '_', 'S', 'P', 'O', 'T', '_', 'M', 'I','C', 'R', 'O'
};
...
/* Service */
static const uint16_t GATTS_SERVICE_UUID_NAVIGATION      = 0x00FF;
static const uint16_t GATTS_CHAR_UUID_SET_ORI       = 0xFF01;
...
static uint8_t orientation_set_value[12]       = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static bool enable_orientation_act_ntf = false;
...
static const esp_gatts_attr_db_t gatt_db[IDX_NB] =
{
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&primary_service_uuid, ESP_GATT_PERM_READ,
      sizeof(uint16_t), sizeof(GATTS_SERVICE_UUID_NAVIGATION), (uint8_t *)&GATTS_SERVICE_UUID_NAVIGATION}},
    ...
    [IDX_CHAR_VAL_A] =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_UUID_SET_ORI, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      GATTS_DEMO_CHAR_VAL_LEN_MAX, sizeof(orientation_set_value), (uint8_t *)orientation_set_value}},
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_UUID_GET_ORI, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      GATTS_DEMO_CHAR_VAL_LEN_MAX, sizeof(orientation_act_value), (uint8_t *)orientation_act_value}},
    ...
    //   GATTS_DEMO_CHAR_VAL_LEN_MAX, sizeof(orientation_sensor_value), (uint8_t *)orientation_sensor_value}},
    ...
    switch (event) {
        ...
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep){
                ...
                if (handle_table[IDX_CHAR_CFG_B] == param->write.handle && param->write.len == 2){
                    uint16_t descr_value = param->write.value[1]<<8 | param->write.value[0];
                    if (descr_value == 0x0001){
                        ESP_LOGI(GATTS_TABLE_TAG, "notify enable");
                        enable_orientation_act_ntf = true;
                        esp_ble_gatts_send_indicate(gatts_if, param->write.conn_id, handle_table[IDX_CHAR_VAL_B],
                                                sizeof(orientation_act_value), orientation_act_value, false);
                    } 
                    else if (descr_value == 0x0000){
                        ESP_LOGI(GATTS_TABLE_TAG, "notify disable ");
                        enable_orientation_act_ntf = false;
                    }else{
                        ESP_LOGE(GATTS_TABLE_TAG, "unknown descr value");
                        esp_log_buffer_hex(GATTS_TABLE_TAG, param->write.value, param->write.len);
                    }

                // } else if (handle_table[IDX_CHAR_CFG_C] == param->write.handle && param->write.len == 2){
                //     uint16_t descr_value = param->write.value[1]<<8 | param->write.value[0];
                //     if (descr_value == 0x0001){
                //         ESP_LOGI(GATTS_TABLE_TAG, "notify enable");
                //         enable_orientation_sensor_ntf = true;
                //         esp_ble_gatts_send_indicate(gatts_if, param->write.conn_id, handle_table[IDX_CHAR_VAL_C],
                //                                 sizeof(orientation_sensor_value), orientation_sensor_value, false);
                //     } 
                //     else if (descr_value == 0x0000){
                ...
                //         esp_log_buffer_hex(GATTS_TABLE_TAG, param->write.value, param->write.len);
                //     }
                } else if (handle_table[IDX_CHAR_VAL_A] == param->write.handle && param->write.len == 12){
                    ESP_LOGI(GATTS_TABLE_TAG, "getting new orientation value");
                    if (new_orientation_cb) 
                    {
...
esp_err_t set_new_orientation_act_value(int16_t omega, int16_t phi, int16_t psi, int16_t xm, int16_t ym, int16_t zm) {
  *(int16_t*) (&orientation_act_value[0])  = (int16_t) omega;
  *(int16_t*) (&orientation_act_value[2])  = (int16_t) phi;
  *(int16_t*) (&orientation_act_value[4])  = (int16_t) psi;
  *(int16_t*) (&orientation_act_value[6])  = (int16_t) xm;
  *(int16_t*) (&orientation_act_value[8])  = (int16_t) ym;
  *(int16_t*) (&orientation_act_value[10]) = (int16_t) zm;

  esp_err_t ret;
  ret = esp_ble_gatts_set_attr_value(handle_table[IDX_CHAR_VAL_B], sizeof(orientation_act_value), orientation_act_value);
  if (ret) {
    ESP_LOGE(GATTS_TABLE_TAG, "set_new_orientation_act_value esp_ble_gatts_set_attr_value failed: %d", ret);
    return ESP_FAIL;
  }

  if (enable_orientation_act_ntf) {
    ESP_LOGD(GATTS_TABLE_TAG, "enable_orientation_act_ntf");
    ret = esp_ble_gatts_send_indicate(profile_tab[PROFILE_APP_IDX].gatts_if, profile_tab[PROFILE_APP_IDX].conn_id, handle_table[IDX_CHAR_VAL_B], sizeof(orientation_act_value), orientation_act_value, false);
    if (ret) {
